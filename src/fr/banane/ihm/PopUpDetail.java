...
public class PopUpDetail extends JDialog {
	...
	public PopUpDetail(JFrame parent, String title, boolean modal, int id){
		...
		reciveData();
		initComponent();
		//setVisible(true); // Cause du double affichage de la popup : déjà fait dans la classe ViewDetailVehiculeListener
	}
	...
	public void initComponent() {
		...
		nom = new JLabel(vehicule.getNom());
		...
		marque = new JLabel(vehicule.getMarque().toString());
		...
		typeMoteur = new JLabel(vehicule.getMoteur().toString());
		...
		prix = new JLabel(vehicule.getPrix().toString());
		...
		option = new JLabel(vehicule.getOptions().toString());
		...
		//Prix total
		prixTot = new JLabel(vehicule.getPrixTotal().toString());
	...
	public void reciveData() {
		...
		vehicule = daoV.find(id);

		System.out.println(vehicule.getNom() + " marque " + vehicule.getMarque());
		logger.debug("id : " + id + " / " + vehicule.toString());
	}
}